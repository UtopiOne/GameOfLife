...
void Grid<width, height>::Draw() {
  ...
  for (int x = 0; x <= width; x++) {
    for (int y = 0; y <= height; y++) {
      if (m_SelectedTileX == x && m_SelectedTileY == y) {
        DrawRectangle(x * CELL_WIDTH, y * CELL_HEIGHT, CELL_WIDTH, CELL_HEIGHT,
                      HOVERED_CELL_COLOR);
      }
  ...
  if (m_CurrentState == GridState::Running) {
    ...
    DrawText(TextFormat("Generation: %i", m_Generation),
             WIDTH / 2 - genSize / 2, 60, 20, TEXT_COLOR);
  }

  if (m_CurrentState == GridState::Editing) {
    int textSize = MeasureText("Editing. Press SPACE to run.", 40);
    DrawText("Editing. Press SPACE to run.", WIDTH / 2 - textSize / 2, 5, 40,
             TEXT_COLOR);
  }
...
void Grid<width, height>::UpdateEditing() {
  if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && !m_MouseJustPressed) {
    CellMatrix[m_SelectedTileX][m_SelectedTileY].IsAlive =
        !CellMatrix[m_SelectedTileX][m_SelectedTileY].IsAlive;
  ...
  for (int i = x - 1; i <= x + 1; i++) {
    for (int j = y - 1; j <= y + 1; j++) {
      if (i == width && CellMatrix[0][j].IsAlive && !(i == x && j == y)) {
        neighboring++;
      } else if (i == -1 && CellMatrix[width - 1][j].IsAlive &&
                 !(i == x && j == y)) {
        neighboring++;
      } else if (j == -1 && CellMatrix[i][height - 1].IsAlive &&
                 !(i == x && j == y)) {
        neighboring++;
      } else if (j == height && CellMatrix[i][0].IsAlive &&
                 !(i == x && j == y)) {
        neighboring++;
      } else if (CellMatrix[i][j].IsAlive && !(i == x && j == y) &&
                 i != width && i != -1 && j != height && j != -1) {
        neighboring++;
      }
    }
  }
